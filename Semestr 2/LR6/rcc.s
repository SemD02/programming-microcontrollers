...
wait_hserdy
	...
	ldr r10,=(1<<PLLSRC) + (4<<PLLMUL);тут в PLLMUL, потому что  (4+2)=6 =>(6*8=48), в умножитель подаем из источника  HSE - в PLLSRC стоит 1
	str r10,[r0]
...
wait_pllrdy
	ldr r10,[r0] 
	tst r10, #(1<<PLLRDY);— флаг готовности PLL, устанавливается аппаратно.
	beq wait_pllrdy
	ldr r0,=SAR+(RCC_CFGR&0x00FFFFFF)*0x20+SW*4;с помощью этих битов можно выбрать источник для тактирования SYSCLK.(HSE-1)
	str r9,[r0]
	BX LR
	ENDP
	EXPORT RCC_INIT
